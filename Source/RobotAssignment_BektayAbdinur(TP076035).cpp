...
void RobotAssignment::updateRobotStatus(string id, string newStatus) {
    for (int i = 0; i < robotCount; i++) {
        if (robots[i].robotID == id) {
            ...
            cout << "[Robot Assignment] Robot " << id << " status updated to " << newStatus << ".\n";
            return;
        }
    }
...
bool RobotAssignment::assignTask(string orderID) {
    ...
    while (checkedRobots < robotCount) {
        ...
        cout << "[Robot Assignment] " << robots[nextIndex].robotID
             << " skipped because status is " << robots[nextIndex].status << ".\n";

        nextIndex = (nextIndex + 1) % robotCount;
        checkedRobots++;
    }
    cout << "[Robot Assignment] No available robot found for Order " << orderID << ".\n";
    return false;
}
    ...
    for (int i = 0; i < robotCount; i++) {
        if (robots[i].robotID == robotID) {
            if (robots[i].status == "Busy") {
                cout << "[Robot Assignment] Robot " << robotID << " completed Order " << robots[i].currentOrderID << ".\n";
                robots[i].status = "Available";
                robots[i].currentOrderID = "-";
            } else {
                cout << "[Robot Assignment] Robot " << robotID << " is not currently busy.\n";
            }
            return;
        }
    }
